layui.define(['message', 'table', 'jquery', 'element', 'yaml', 'form', 'tab', 'menu', 'frame', 'theme', 'convert'],
	function (exports) {
		...
		var messageTip = function (id, title, context, form, type) {
			$.ajax({
				headers: {
					"Authorization": 'bearer ' + localStorage.getItem("token")
				},
				url: api + "/api/member/memberNewsUpdate/" + id,
				// data: { id: backreason },
				dataType: 'json',
				type: 'get',
				success: function () {
			...
			if (type == 1) {
				layer.open({
					...
					content: "<div style='background-color:whitesmoke;'><div class='layui-card'><div class='layui-card-body'>来源 : &nbsp; " +
						...
						"</div><div class='layui-card-header' >内容 : &nbsp; " + context + "</div>" +
						'<div class="layui-card-header" style="margin-left: -13%;margin-top: 1.5%;">' +
						'<label class="layui-form-label">回复 : &nbsp; </label>' +
						'<div class="layui-input-block">' +
						'<input style="margin-left: -4%;width: 308px;" type="text" id="sendcontent" class="layui-input">' +
						'</div>' +
						'</div>' +
						"</div></div>", //支持获取DOM元素
					btn: ['发送'], //按钮组
					...
					yes: function (index, layero) { //layer.msg('yes');    //点击确定回调
						var backreason = layero.find('input')[0].value;
						$.ajax({
							headers: {
								"Authorization": 'bearer ' + localStorage.getItem("token")
							},
							url: api + "/api/member/memberNewsAdd",
							data: { opid: id, content: backreason },
							dataType: 'json',
							type: 'post',
							success: function (result) {
								if (result.code == '200') {
